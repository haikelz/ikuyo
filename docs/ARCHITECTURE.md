...
Ikuyo is a Bun/Moon monorepo. `apps/web` is a static Astro 6 application with
Svelte 5 islands, Tailwind CSS 4, and typed MDX content collections.

## Boundaries

- `apps/web/src/pages`: route composition and static-path generation only.
- `apps/web/src/content.config.ts`: collection schemas; content enters the app here.
- `apps/web/src/content`: MDX records and frontmatter, one record per public item.
- `apps/web/src/components`: shared presentation; Svelte is reserved for interaction.
- `packages/ui`: reusable shadcn-svelte primitives.
- `apps/web/cypress`: browser evidence for public behavior.
- `docs` and `scripts`: Harness workflow and its local durable state; they do not own website runtime behavior.

## Content route pattern

Dynamic content routes use `getCollection`, return one static path per entry,
render the selected entry with `render`, and pass its metadata into the shared
`Layout`. Do not duplicate collection data in a page module.
