  ...
  >
    <div class="fixed">
      <div class="rounded-md bg-neutral-900 p-4">
        <p class="text-justify font-semibold text-basetext-neutral-50">
...
<button
  class="rounded-md bg-neutral-900 cursor-pointer p-2 fixed bottom-4 left-4"
  type="button"
  aria-label="photos info"
  onclick={togglePhotosInfo}
>
  {#if isOpen}
    <XIcon size={18} class="text-neutral-50" />
  {:else}
    <InfoIcon size={18} class="text-neutral-50" />
  {/if}
</button>
