...
{#if isLoadingMarkets}
  <Alert class="rounded-none">
    ...
    <AlertDescription
      >Mohon tunggu, sedang mengambil data terbaru.</AlertDescription
    >
  </Alert>
    ...
    <Card class="border border-border/70 bg-transparent rounded-none">
      ...
      <CardContent class="space-y-4">
        ...
        <p class="text-xs text-muted-foreground">
          Last updated: {dateFormatter.format(
            new Date(currentMarket.fetchedAt),
    ...
    <Dialog bind:open={isChartDialogOpen}>
      ...
      >
        <div class="flex h-full min-h-0 w-full flex-col">
          ...
          <div
            class="flex-1 min-h-0 overflow-y-auto p-3 pb-[max(1rem,env(safe-area-inset-bottom))] md:p-4"
          >
